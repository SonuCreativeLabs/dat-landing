...
const ContactMessages = ({ archived = false }: ContactMessagesProps) => {
  console.log("ContactMessages rendered with archived =", archived);
  
  useEffect(() => {
    console.log("ContactMessages mounted/updated with archived =", archived);
  }, [archived]);
  ...
  const archiveEnquiry = useMutation({
    mutationFn: async (id: string) => {
      console.log("Starting archive mutation for id:", id);
    ...
    >
        ...
        <div className="flex-shrink-0 pt-1">
          {!archived && (
            <Button
              ...
              size="sm"
              className="mr-2"
              onClick={() => archiveEnquiry.mutate(message.id)}
            >
      ...
      {archived && (
        <Button
          ...
          size="sm"
          className="mt-4"
          onClick={() => unarchiveEnquiry.mutate(message.id)}
        >
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {messages.map((message) => message_card(message))}
      </div>
      {messages.length === 0 && (
        <div className="text-center py-8 text-gray-500 bg-gray-50 rounded-lg">
          {archived ? "No archived enquiries found" : "No enquiries yet"}
        </div>
      )}
    </div>
  );
